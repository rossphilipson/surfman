...
static struct drm_plane *k2000_plane_new(struct drm_framebuffer *framebuffer, uint32_t crtc_id)
{
...
static int k2000_plane_set(struct drm_plane *plane)
{
...
static void k2000_plane_release(struct drm_plane *plane)
{
...
static struct drm_framebuffer *k2000_framebuffer_new(struct drm_device *device, struct drm_surface *surface)
{
...
static int k2000_framebuffer_release(struct drm_framebuffer *framebuffer)
{
 ...
 * Device interface.
 */
static int k2000_modeset(struct drm_monitor *monitor, struct drm_framebuffer *drmfb)
{
    int rc = 0;
    ...
    if (!mode) {
        ...
        if (fallback_mode) {
            ...
            monitor->plane = k2000_plane_new(drmfb, monitor->crtc);
            if (monitor->plane) {
                ...
                k2000_plane_release(monitor->plane);
                monitor->plane = NULL;
            }
        }
...
modeset:
    ...
    if (monitor->plane) {
        rc = k2000_plane_set(monitor->plane);
        if (rc) {
...
static int k2000_set(struct drm_monitor *monitor, struct drm_surface *surface)
{
    int rc;
    struct drm_framebuffer *drmfb;

    drmfb = k2000_framebuffer_new(monitor->device, surface);
    if (!drmfb) {
        rc = -errno;
...
static void k2000_unset(struct drm_monitor *monitor)
{
    ...
    if (monitor->framebuffer) {
        k2000_framebuffer_release(monitor->framebuffer);
...
static void k2000_refresh(struct drm_monitor *monitor, const struct drm_surface *surface,
...
static int k2000_match_udev_device(struct udev *udev, struct udev_device *device)
{
    ...
    devid_str = udev_device_get_sysattr_value(dev, "device");
    if (devid_str)
...
const struct drm_device_ops k2000_ops = {
    .set = k2000_set,
    .unset = k2000_unset,
    .refresh = k2000_refresh,
    .match = k2000_match_udev_device
};
